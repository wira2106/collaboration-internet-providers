<template>
  <div class="btn-group btn-group-pengaturan">
    ...
    <button
      type="button"
      class="btn btn-default btn-list-pengaturan"
    ...
    <div
      class="dropdown-menu"
      style="transform: translate3d(-120px, 160px, 0px)!important;min-width:0px!important;"
    >
      <h6 class="dropdown-header" style="text-align:center">
        {{ trans("presales.title.sidebar") }}
      </h6>
          ...
          <div class="row justify-content-center">
            <label class="switch">
              <input
                ...
                id="CheckBoxRumah"
                v-model="presale"
                @change="showHideRumah()"
              />
              <span class="slider round-2" id="CheckBoxRumahSlider"></span>
            </label>
          </div>
        </div>
      </a>
          ...
          <div class="row justify-content-center">
            <label class="switch">
              <input
                type="checkbox"
                v-model="endpoint"
                @change="showHideEndpoint()"
              />
              <span class="slider round-2" id="CheckBoxEndpointSlider"></span>
            </label>
          </div>
        </div>
      </a>
  ...
  methods: {
    showHideRumah() {
      if (this.presale) {
        ...
        this.$emit("show_marker_rumah", true);
      } else {
        ...
        this.$emit("show_marker_rumah", false);
      }
    },
    showHideEndpoint() {
    ...
    send_confirmation() {
      ...
      axios
        .post(
          route("api.presale.presales.email_confirmation", {
