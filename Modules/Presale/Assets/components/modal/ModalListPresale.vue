<template>
  <bs-modal id="modal-list-presale" size="large" @onClose="onClose()">
    <div slot="header">
    ...
    <div class="row">
      ...
      <div class="col-6">
        <div class="form-group">
          <button
            class="btn btn-sm btn-primary showSearched"
            @click="showSearched"
            type="button"
          >
            {{ trans("presales.form.show") }}
          </button>
        </div>
      </div>
    </div>
    ...
    <div class="col-12">
      ...
      <div
        class="content-menu-rumah-right"
        style="max-height: 450px;overflow-y: auto;"
      >
        <table
          style="width:100%;margin-bottom:0px;"
          class="table table-hover table-list-rumah"
          v-if="list_presales.length > 0"
        >
          <tr
          ...
          >
            <td
              valign="center"
              style="width:12px;padding:.75rem 0 0 .30rem;padding-top:25px;"
              align="center"
            >
            ...
            <td style="padding:.75rem .75rem .75rem .20rem">
              ...
              <span
                class="span-class"
                style="font-size:8px;margin-top:4px;float:right"
              >
                {{ list.created_at }}
              </span>
              ...
              <i
                class="span-alamat"
                style="font-size:9px;display:block;line-height:1;padding-left:5px;"
              >
                {{ list.address }}
              </i>
            </td>
        ...
        <p
          ...
          v-if="list_presales.length == 0 && loading == false"
        >
          {{ trans("presales.form.no data") }}
        </p>
        <center>
          <i id="loading-text-list-rumah" v-if="loading"
            >{{ trans("core.loading") }}
          </i>
        </center>
      </div>
...
export default {
  ...
  methods: {
    ...
    fetchListPresale(url = route("api.presale.presales.list")) {
      ...
      axios.get(url, properties).then((response) => {
        if (response) {
          this.list_presales = [...this.list_presales, ...response.data.data];
    ...
    showSearched() {
      ...
      axios
        .get(route("api.presale.presales.list"), properties)
        .then((response) => {
  ...
  mounted() {
    ...
    $(".content-menu-rumah-right").scroll(() => {
      if (
        $(".content-menu-rumah-right").scrollTop() + 10 >=
        $(".table-list-rumah").height() -
          $(".content-menu-rumah-right").height()
      ) {
