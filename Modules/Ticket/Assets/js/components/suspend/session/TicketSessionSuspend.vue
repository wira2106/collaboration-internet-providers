...
import AcceptSLAVue from "../partials/AcceptSuspend.vue";
...
export default {
  ...
  methods: {
    ...
    onSubmit(formName) {
      ...
      Swal.fire({
        ...
        if (result.value) {
          ...
          this.form_data
            .post(
              route("admin.api.ticket.suspend.session.message.send", {
    ...
    fetchData(whitoutchat = 0) {
      ...
      axios
        .get(
          route("admin.api.ticket.suspend.session.detail", {
    ...
    fetchMessages(scroll = 1) {
      var message = this.data.messages[this.data.messages.length - 1];
      const properties = {
        params: {
          id_terakhir: typeof message == "undefined" ? "" : message.message_id,
        },
      };
      if (this.request == null) {
        ...
        this.request = axios
          .get(
            route("admin.api.ticket.suspend.session.message.get", {
              ticket_id: this.$route.params.id,
            }),
            _.merge(properties)
          )
    ...
    closeTicketSupport() {
      ...
      if (this.data.accept_isp_by == null) {
        text = this.trans("ticketsSuspend.messages.suspend tidak muncul.isp");
      } else if (this.data.accept_osp_by == null) {
        text = this.trans("ticketsSuspend.messages.suspend tidak muncul.osp");
      }
      Swal.fire({
        title: this.trans("ticketsSuspend.alert.close ticket"),
        text: text,
        icon: "warning",
        showCancelButton: true,
        confirmButtonColor: "#3085d6",
        ...
        if (result.value) {
          ...
          axios
            .post(
              route("admin.api.ticket.suspend.session.close", {
                ticket_id: this.$route.params.id,
              })
            )
            .then((response) => {
