  ...
  methods: {
      ...
      axios
        ...
        .then((response) => {
          ...
          if (this.surveyForm.status == "active") {
            this.statusEditFormSurvey = false;
          }
          // slot instalasi
          ...
          // check local data
          let data = localStorage.getItem("data_fill"),
            getSurveyForm = null;
          if (data !== null && data !== "") {
            getSurveyForm = JSON.parse(data);
            this.surveyForm.jadwalInstalasi = [];
            this.surveyForm.perangkat_tambahan = [];
            if (
              getSurveyForm.mode == "survey_form" &&
              getSurveyForm.data.pelanggan_id == this.surveyForm.pelanggan_id
            ) {
              getSurveyForm.data.jadwalInstalasi.forEach((teknsi) => {
                this.surveyForm.jadwalInstalasi.push(teknsi);
              });
              getSurveyForm.data.perangkat_tambahan.forEach((perangkat) => {
                this.surveyForm.perangkat_tambahan.push(perangkat);
              });
              getSurveyForm.data.teknisiInstalasi.forEach((teknisiIns) => {
                if (teknisiIns !== "new") {
                  this.surveyForm.teknisiInstalasi.push(teknisiIns);
                }
              });
              this.surveyForm.keterangan = getSurveyForm.data.keterangan;
              this.surveyForm.keterangan = getSurveyForm.data.keterangan;
            }
          }
          ...
          //motode penampilan form pertama kali
          this.status_firstLoad();
        });
    },
    ...
    send() {
      ...
      Swal.fire({
        ...
        confirmButtonText: this.trans("surveys.messages.simpan"),
        ...
        if (result.value) {
          ...
          if (this.status_checkbox == true) {
            this.surveyForm.status = "finish";
          } else {
            this.surveyForm.jadwalInstalasi = [];
            this.surveyForm.teknisiInstalasi = [];
            this.surveyForm.status = "active";
          }
          ...
          this.form
            ...
            .then((response) => {
              ...
              let check = localStorage.getItem("data_form_survey");
              if (check !== null || check !== "") {
                this.removeFormDataLocal();
              }
    ...
    successSignal(file, fileList) {
      this.surveyForm.foto_signal_kabel.push({
        name: file.name,
        url: file.url,
        new: true,
      });
      this.surveyForm.upload_signal_kabel.push(file.raw);
    },
    successKabel(file, fileList) {
      this.surveyForm.foto_jalur_kabel.push({
        name: file.name,
        url: file.url,
        new: true,
      });
      this.surveyForm.upload_jalur_kabel.push(file.raw);
    },
    saveFormToLocal(dataFill) {
      ...
      localStorage.setItem("data_fill", data);
    },
    removeFormDataLocal() {
      localStorage.removeItem("data_fill");
    },
    status_firstLoad() {
      ...
      if (
        ...
        this.display_button_batal = false;
      }
    },
  },
  mounted() {
