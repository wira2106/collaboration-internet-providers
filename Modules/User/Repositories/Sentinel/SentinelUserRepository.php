...
class SentinelUserRepository implements UserRepository
{
     ...
     */
    public function serverPaginationFilteringFor(Company $company, Request $request): LengthAwarePaginator
    {
        $roles = $this->allWithBuilder()
                ->join('user_companies', 'user_companies.user_id', 'users.id')
                ->where('user_companies.company_id', $company->company_id)
                ->where(function($roles) use ($request){
                    if ($request->get('search') !== null) {
                        $term = $request->get('search');
                        $roles->where('users.full_name', 'LIKE', "%{$term}%")
                    ...
                    if ($request->get('order_by') !== null && $request->get('order') !== 'null') {
                        ...
                        $roles->orderBy("users.".$request->get('order_by'), $order);
                    } else {
