...
class AuthController extends BasePublicController
{
    ...
    public function postLogin(LoginRequest $request)
    {
        ...
        if(!$this->auth->user()->userCompanies) return $this->getLogout();
    ...
    public function getResetPasswordComplete($id){
        $data = DB::table('users')->where('id',$id)->where('first_login',0)->first();
        if($data){
            return view('user::public.reset.reset_password_company',[
    ...
    public function postResetPasswordComplete(Request $request , $userId , $code){
        try {
            if($request->password != $request->re_password){
                return redirect()->back();
            }
            app(UserResetter::class)->finishResetPasswordCompany(
                array_merge($request->all(), ['userId' => $userId, 'code' => $code])
            );

            //update status first login 
