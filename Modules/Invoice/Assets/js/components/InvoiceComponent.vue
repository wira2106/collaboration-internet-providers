<template>
  <div class="card card-body printableArea" v-loading="loading">
    <h3>
      <span>
        <b>{{ trans("invoices.title.invoices") }}</b>
        <template v-if="this.$route.name != 'admin.invoice.invoices.print'">
          <el-button
            type="success"
            size="small"
            v-if="invoice.status == 'success'"
          >
            {{ trans("invoices.lunas") }}
          </el-button>
          <el-button
            type="warning"
            size="small"
          ...
          >
            {{ trans("invoices.pending") }}
          </el-button>

          <el-button type="danger" size="small" v-if="invoice.status == null">
            {{ trans("invoices.belum bayar") }}
          </el-button>
        </template>
      </span>
    ...
    <div class="row">
      <div class="col-md-12">
        <div class="pull-left">
          <address>
            <h3 style="max-width:450px">
              &nbsp;<b class="text-danger">{{ invoice.nama_tagihan }}</b>
            </h3>
            <h4 class="font-bold">{{ invoice.from.name }}</h4>
        ...
        <div class="pull-right text-right">
          <address>
            ...
            <p class="m-t-30">
              <b>{{ trans("invoices.tanggal invoice") }} :</b>
              <i class="fa fa-calendar"></i> {{ invoice.created_at }}
            </p>
            <p>
              <b>{{ trans("invoices.expired") }} :</b>
              <i class="fa fa-calendar"></i> {{ invoice.jatuh_tempo }}
            </p>
          </address>
        </div>
      </div>
          ...
          <table class="table table-hover">
            ...
            <tbody>
              <tr v-for="(item, i) in item_invoice" :key="i">
                ...
                <td>
                  <span v-if="invoice.type === 'pelanggan'">
                    {{
                      trans("invoices.tagihan pelanggan", { name: item.name })
                    }}
                  </span>
                  <span v-if="invoice.type !== 'pelanggan'">
                    {{ item.name }}
                ...
                <td class="text-right">
                  {{ item_invoice.qty ? item_invoice.qty : 1 }}
                </td>
      ...
      <div class="col-md-12">
        <div class="pull-right m-t-30 text-right">
          <p>
            {{
              trans("invoices.ppn", {
                ppn: invoice.ppn,
                amount: rupiah(total_pajak),
              })
            }}
          </p>
          <hr />
          <h3>
            <b>{{ trans("invoices.total") }} :</b>
            {{ rupiah(total + total_pajak) }}
          </h3>
        </div>
        ...
        <div
          ...
          v-if="this.$route.name != 'admin.invoice.invoices.print'"
        >
          <button
            class="btn btn-info"
            type="submit"
          ...
          <button
            ...
            type="button"
            @click="goToPrint(invoice.invoice_id)"
          >
            <span
              ><i class="fa fa-print"></i> {{ trans("invoices.print") }}</span
            >
          </button>
        </div>
      </div>
    ...
    <div class="card-footer">
      <p style="text-align:center; margin-bottom:0">
        {{ trans("core.powered by openaccess") }}
      </p>
    </div>
  </div>
</template>
