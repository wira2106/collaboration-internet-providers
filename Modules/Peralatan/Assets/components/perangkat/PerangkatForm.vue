                ...
                <div class="row">
                  <div class="col-md-8 offset-md-2">
                    ...
                    >
                      <el-input
                        :placeholder="
                          trans('perangkats.placeholder.perangkats')
                        "
                        v-model="nama_perangkat.value"
                      >
                        <el-button
                          v-if="index == 0 && !$route.params.perangkat"
                          type="primary"
                          slot="append"
                          @click="addInput"
                        >
                        ...
                        <el-button
                          ...
                          type="danger"
                          slot="append"
                          @click="removeInput(nama_perangkat)"
                        >
                ...
                <div class="row">
                  <div class="col d-flex justify-content-center">
                    <el-button type="primary" @click="onSubmit">
                      {{ trans("perangkats.button.simpan perangkat") }}
                    </el-button>
                    <el-button type="danger" @click="$router.go(-1)">
                      {{ trans("perangkats.button.kembali") }}
                    </el-button>
                  </div>
                </div>
              </div>
  ...
  methods: {
    onSubmit() {
      ...
      this.$refs["perangkatForm"].validate((valid) => {
        if (valid) {
          Swal.fire({
            title: this.trans("perangkats.messages.simpan perangkat"),
            text: "",
            icon: "warning",
    ...
    fetchData() {
      ...
      if (params !== undefined) {
        let routeUrl = route("api.peralatan.perangkat.find", {
          perangkat: params,
        });
        axios.get(routeUrl).then((response) => {
          this.perangkatForm.nama_perangkat[0].value =
            response.data.nama_perangkat;
          this.loading = false;
        });
      } else {
        this.loading = false;
