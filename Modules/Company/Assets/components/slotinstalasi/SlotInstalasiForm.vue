    ...
    <div class="row page-titles">
      ...
      <div class="col-md-7 align-self-center">
        <ol class="breadcrumb">
          ...
          <li class="breadcrumb-item">
            {{ trans(`slot_instalasi.${pageTitle}`) }}
          </li>
        </ol>
      </div>
    </div>
      ...
      <div class="card card-outline-info">
        <div class="card-header text-white">
          ...
          <el-button
            ...
            @click="addSlotInstalasi()"
            >{{
              trans("slot_instalasi.button.create slotinstalasi")
            }}</el-button
          >
        </div>
        <div class="card-body">
          ...
          >
            ...
            >
              <template slot-scope="scope">
                <el-button-group>
                  <el-tooltip
                    :content="trans('core.tooltip.slot.edit')"
                    placement="top"
                  >
                    <el-button
                      icon="el-icon-edit"
                      @click="goToEdit(scope)"
                      size="mini"
                    >
                    </el-button>
                  </el-tooltip>
                  <el-tooltip
                    :content="trans('core.tooltip.slot.hapus')"
                    placement="top"
                  >
                    <delete-button
                      :scope="scope"
      ...
      <div class="modal-dialog">
        <div class="modal-content">
          ...
          <div class="modal-footer">
            <el-button type="danger" size="small" data-dismiss="modal">
...
export default {
  ...
  data() {
    return {
      ...
      rulesAddSlot: {
        required: true,
        message: this.trans("slot_instalasi.validation.required", {
          field: "Nama slot",
        }),
      },
      selectableRange: [],
  ...
  methods: {
    modalTitle() {
      return this.processForm === "edit"
        ? this.trans("slot_instalasi.title.edit")
        : this.trans("slot_instalasi.title.create");
    },
    ...
    queryServer(customProperties) {
      ...
      axios
        .post(
          route(
            "api.company.slotinstalasi.index",
    ...
    goToEdit(scope) {
      ...
      this.form_slot = {
        ...
        name: scope.row.name,
        jam: [
          new Date("2021-01-01 " + scope.row.jam_start),
          new Date("2021-01-01 " + scope.row.jam_end),
        ],
      };
    },
    dayFormatted() {
      this.form_slot.jam_start = this.form_slot.jam[0].toLocaleTimeString(
        "it-IT"
      );
      this.form_slot.jam_end = this.form_slot.jam[1].toLocaleTimeString(
        "it-IT"
      );
    },
    saveSlotInstalasi(formName) {
      ...
      this.$refs[formName].validate((valid) => {
        if (valid) {
          ...
          axios
            .put(route("api.company.slotinstalasi.update"), properties)
            .then((response) => {
    ...
    fetchDateRangeSlot() {
      axios
        ...
        .then((response) => {
          ...
          this.form_slot.jam = [
            new Date(`2019 ${this.selectableRange[0]}`),
            new Date(`2019 ${this.selectableRange[1]}`),
          ];
        })
        .catch((err) => {
          setTimeout(() => {
