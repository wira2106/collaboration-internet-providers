          ...
          <div class="col-12 " v-if="picked_day.length > 0">
            ...
            >
              <a
                href="#!"
                class="pb-2"
                style="font-weight: 400;padding: 0px 15px;color:white!important;"
              >
                {{ picked_day.length }}
                {{ trans("companies.title.day selected") }}</a
              >

              <el-tooltip
                :content="trans('core.tooltip.day off.tambah')"
                placement="top"
              >
                <button
                  type="button"
                ...
                >
                  ...
                  {{ trans("companies.title.add dayoff") }}
                </button>
              </el-tooltip>
            </div>
          </div>
          <div class="col-12">
            <el-calendar :key="keyCalendar">
              <template slot="dateCell" slot-scope="{ data }">
                <div
                  class="el-calendar-day__child"
                  style="background-color:red"
                  v-if="findDayOf(data.day)"
                  @click="addDayOff(data, 'dayon')"
                >
                  <p class="is-selected" style="color:white">
                    {{ data.day.split("-")[2] }}
                    {{ findDayOf(data.day).deskripsi_libur }}
                  </p>
                </div>
                <div
                  v-else
                  class="el-calendar-day__child"
                  :style="
                    picked_day.indexOf(data.day) != -1
                      ? 'background-color: #c1c6f7;'
                      : ''
                  "
  ...
  methods: {
    addDayOff(data, type) {
      ...
      if (type === "dayoff") {
        ...
        this.$confirm(
          this.trans("dayoff.destroy"),
          this.trans("companies.title.dayoff"),
          {
          ...
          .then(() => {
            ...
            form
              .post(
                route("api.company.company.dayon", { company: this.company_id })
              )
              .then((response) => {
                this.fetchDayOff();
                this.Toast({
    ...
    tambahDayOff() {
      let self = this;
      this.$prompt("Masukkan deskripsi hari libur", "Description", {
        ...
        .then(({ value }) => {
          ...
          form
            .post(
              route("api.company.company.dayoff", { company: this.company_id })
            )
            .then((response) => {
              this.fetchDayOff();
              this.picked_day = [];
    ...
    fetchDayOff() {
      ...
      axios
        .get(
          route("api.company.company.dayoff.index", {
