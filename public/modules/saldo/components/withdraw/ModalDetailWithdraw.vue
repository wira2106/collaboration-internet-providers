<template>
    <div>
        <div class="modal-header" style="margin-top:-65px;">
            <div class="pull-left">
               <h3><b>{{trans('saldos.modal.modal withdraw')}}</b></h3> 
            </div>
        </div>
                  ...
                  <el-form 
                    ...
                    <div class="row">
                        <div class="col-md-12">
                            <ul class="list-group">
                                ...
                                <li class="list-group-item" style="border: 0px;">
                                    <table class="table">
                                         <tr>
                                            <td style="font-size: 14px;">{{trans('saldos.modal.jmlh penarikan')}}</td>
                                            <td>
                        ...
                        <div class="col-md-12 text-center" 
                          v-if="data.status == 'pending' && tarik_saldo_id[1] == 'Super Admin'">
                            <label>{{trans('saldos.form.bukti')}}</label>
                            <el-form-item class="upload-image-withdraw" prop="bukti_transfer">
                                <upload-avatar
                    ...
                    <div class="row text-center" v-if="data.status !== 'success' && data.status !== 'cancel' && tarik_saldo_id[1]== 'Super Admin'">
                        <div class="col-md-12 pb-1">
                            <el-button type="primary" style="width: 350px;" @click="onSubmit('success')">{{trans('saldos.button.konfirmasi')}}</el-button>
                        </div>
                        <div class="col-md-12">
                            <el-button type="danger" style="width: 350px;" @click="reject(true,false)">{{trans('saldos.button.batal')}}</el-button>
                        </div>
                    </div>
                  </el-form>
                </div>
    ...
    export default{
        ...
        data(){
            return{
                ...
                rules:{
                    keterangan: [
                        { required: true, message: this.trans('saldos.validation.required',{field:this.trans('saldos.form.alasan')}), trigger: 'change' }
                    ],
                     bukti_transfer: [
        ...
        methods:{
            ...
            confirm(){
                 Swal.fire({
                    ...
                    this.form.post(route('api.saldo.withdraw.update')).then((response)=>{
                            this.Toast({
                                icon: "success",
