<template>
    <div>
        <div class="modal-header" style="margin-top:-65px;">
            <div class="pull-left">
               <h3><b>{{trans('saldos.modal.tarik saldo')}}</b></h3> 
            </div>
        </div>
            ...
            <div class="row">                
                <div class="col-md-12">
                    <el-form 
                        ...
                        :rules="rules" 
                        ref="withdrawForm">
                        <el-form-item :label="trans('saldos.form.rek')" prop="bank_account_id" size="medium">
                          <el-select v-model="withdrawForm.bank_account_id" style="display:block;">
                            <el-option
                                v-for="rek in selectRek"
                                :label="rek.namaBank"
                                :value="rek.bank_account_id"
                                :key="rek.bank_id">
                                {{rek.namaBank}} ( {{rek.rekening}} )
                            </el-option>
                            <el-option :label="trans('saldos.form.tambah rekening')" value="new"></el-option>
                          </el-select>
                        </el-form-item>
                        ...
                        <el-form-item>
                            <el-checkbox v-model="checked" @change="withdrawAll">{{trans('saldos.form.tarik')}}</el-checkbox>
                        </el-form-item>
                    </el-form>
                </div>
            </div>
        ...
        <div class="modal-footer">
            <el-button size="small" :loading="modalLoading" data-dismiss="modal">{{trans('saldos.button.cancel')}}</el-button>
            <el-button size="small" :loading="modalLoading" type="primary" icon="el-icon-upload" @click="onSubmit()">{{trans('saldos.button.simpan')}}</el-button>
        </div>
    </div>
</template>
        ...
        watch:{
            "withdrawForm.bank_account_id": function(val){
              if (val == 'new') {
                ...
                let vm = this;
                setTimeout(function() {
                  vm.$router.push({name:'admin.company.company.edit', 
        ...
        methods:{
                    ...
                    if (valid) {
                        Swal.fire({
                            ...
                            if (result.value) {
                                ...
                                this.form.post(route('api.saldo.withdraw.store')).then((response)=>{
                                    if (response.errors) {
                                      this.Toast({
                                          icon: "error",
            ...
            validAmount(rule, value, callback){                
                if(parseInt(value) > this.saldo){
                  return callback(new Error(this.trans('saldos.validation.withdraw saldo')));
                }else if(parseInt(value) <= 0){
                  return callback(new Error(this.trans('configurations.validation.invalid number')));
                }
                callback();
            },
            numberFormat(){
                ...
                if(isNaN(this.withdrawForm.amount) && this.withdrawForm.amount.length ==3) this.withdrawForm.amount = 0;
