...
<div>
    <div class="row page-titles">
        <div class="col-md-5 align-self-center">
            <h3 class="text-themecolor" @click="fetchData"> {{ trans('mutasi.list resource') }}</h3>
        </div>
        <div class="col-md-7 align-self-center">
            <ol class="breadcrumb">
                <li class="breadcrumb-item">
                    <a href="/backend/saldo/mutasi">{{ trans('mutasi.list resource') }}</a>
                </li>
            </ol>
        </div>
    </div>
                    ...
                    <div class="tool-bar row" style="padding-bottom: 20px;">
                        <div class="col-12">
                            <div class="row">
                                <div class="col-md-9">
                                    <div class="row">
                                        <div class="col-md-6 pb-3" v-if="user_role != '' && user_role == 'Super Admin'">
                                            <el-select v-if="companies.length > 0" v-model="company_id" placeholder="Select" filterable size="small" @change="fetchData">
                                                <el-option v-for="item in companies" :key="item.company_id" :label="setLabel(item)" :value="item.company_id" :data-id="item.company_id">
                                                </el-option>
                                            </el-select>
                                        </div>
                                        <div class="col-md-4 text-center pb-3">
                                            <el-date-picker size="small" v-model="periode" type="month" format="MMM yyyy" value-format="yyyy-MM" @change="fetchData" :placeholder="trans('mutasi.placeholder.periode')">
                                            </el-date-picker>
                                        </div>
                                    </div>
                                </div>
                        ...
                        <div class="col-12">
                            <el-table :data="data" stripe style="width: 100%" ref="pageTable" v-loading.body="tableIsLoading" @sort-change="handleSortChange">
                                ...
                                <el-table-column width="150" prop="tanggal" :label="trans('mutasi.table.tanggal')">
                                    <template slot-scope="scope">
                                        {{ scope.row.tanggal }}<br>{{scope.row.jam}}
                                    </template>
                                </el-table-column>
                                <el-table-column prop="deskripsi" :label="trans('mutasi.table.deskripsi')">
                                    <template slot-scope="scope">
                                        {{scope.row.deskripsi}}
                                    </template>
                                </el-table-column>
                                <el-table-column width="120" prop="debit" :label="trans('mutasi.table.debit')">
                                    <template slot-scope="scope">
                                        <span v-if="scope.row.tipe == 'debit'">{{scope.row.nominal_rp}}</span>
                                        <span v-else> </span>
                                    </template>
                                </el-table-column>
                                <el-table-column width="120" prop="kredit" :label="trans('mutasi.table.kredit')">
                                    <template slot-scope="scope">
                                        <span v-if="scope.row.tipe == 'credit'">{{scope.row.nominal_rp}}</span>
                                        <span v-else> </span>
                                    </template>
                                </el-table-column>
                                <el-table-column width="130" prop="saldo_akhir" :label="trans('mutasi.table.saldo akhir')">
                                    <template slot-scope="scope">
...
export default {
    ...
    methods: {
        fetchCompany() {
            axios.post(route('api.company.company.list'))
                .then((response) => {
        ...
        queryServer(customProperties) {
           const cancelSource = axios.CancelToken.source();
            ...
            axios.get(route('api.saldo.mutasi.pagination'), _.merge(properties, customProperties))
                .then((response) => {
        ...
        fetchSaldoMutasi() {
            ...
            axios.post(route('api.saldo.mutasi.getsaldo'), data)
                .then((response) => {
