...
export default {
       ...
       resultWilayah(){
         if(this.cari_wilayah){
           return this.wilayah.filter((item)=>{
             ...
             let condition1 = this.cari_wilayah.toLowerCase().split(' ').every(v => item.name.toLowerCase().includes(v));
             let condition2 = this.cari_wilayah.toLowerCase().split(' ').every(v => item.company_name.toLowerCase().includes(v));
             let condition3 = this.cari_wilayah.toLowerCase().split(' ').every(v => item.kabupaten.toLowerCase().includes(v));
             let condition4 = this.cari_wilayah.toLowerCase().split(' ').every(v => item.provinsi.toLowerCase().includes(v));
   ...
   watch:{
    ...
    methods: {
        ...
        fetchWilayah(){
          let self = this;
          axios.post(route('api.wilayah.wilayah.load_titik'))
            .then((response) => {
        ...
        async infoWilayah(wilayah_id){
          ...
          this.map.setZoom(18);          
          this.infoWindow.setContent(`<center><div class="spinner-border text-info" role="status"></div></center>`);
          ...
          await this.detailWilayah(wilayah_id).then((val) => {
            ...
            let content = `<div>`;
                ...
                content += `<center><b><h3>${val.name}</h3></b><span>${val.company_name}</span></center>
                              <hr style="margin:5px 0px">
                              <div>
                                  <table style="width:100%;text-align:center;">
                                      <tbody>
                                        <tr>
                                          <td style="wdith:50%"><h3>${val.end_point}</h3><span>${this.trans('pengajuans.title.end point')}</span></td>
                                          <td><h3>${val.site}</h3><span>${this.trans('pengajuans.title.cover site')}</span></td>
                                        </tr>
                                      </tbody>
                                  </table>
                              </div><br>
                              <div style="width:100%;">
                                <center>
                                  <button type="button" onclick="showCoverage(${val.wilayah_id})" class="btn btn-info btn-circle">
                                    <i class="fa fa-map-marker"></i>
                                  </button>
                                  <button type="button" onclick="detailPengajuan(${val.wilayah_id})" class="btn-pengajuan-marker btn btn-success btn-circle">
        ...
        showDetailPengajuan(wilayah_id){
          ...
          axios.post(route('api.wilayah.wilayah.find',{wilayah:wilayah_id}),_.merge(properties))
            .then((response) => {
        ...
        fetchEndPoint(wilayah_id)
        {
          ...
          axios.post(route('api.presale.endpoint.location'), {wilayah_id: wilayah_id})
          .then(response => {
        ...
        {
          this.loading_map = true;
          axios.post(route('api.presale.presales.location'), {wilayah_id: wilayah_id})
          .then(response => {
      ...
      showDetailCompany(company_id){
        this.modalPengajuan.loading = true;
        axios.post(route('api.company.company.find',{company:company_id}))
        .then((response) => {
          this.modalPengajuan.loading = false;
          ...
          if (valid) {
            Swal.fire({
              ...
              if (result.value) {
                ...
                form.post(route('api.wilayah.pengajuan.submit',{wilayah_id:wilayah_id}))
                .then((response) => {
