...
<div>
        ...
        <div class="col-md-7 align-self-center">
            <ol class="breadcrumb">
                ...
                <li class="breadcrumb-item">
                    <router-link :to="{name: 'admin.configuration.bank.index'}"> {{ trans('bank.title.bank') }} </router-link>
                </li>
            </ol>
        </div>
    </div>
            ...
            <div class="card-body">
                <div class="sc-table">
                    ...
                    <el-table :data="data" stripe style="width: 100%" ref="pageTable" v-loading.body="tableIsLoading" @sort-change="handleSortChange">
                        ...
                        <el-table-column prop="logo" :label="trans('bank.table.logo')" sortable="custom">
                            <template slot-scope="scope">
                                <a @click.prevent="goToEdit(scope)" href="#">
                                    <SmallImage :url="scope.row.logo_url">
                                    </SmallImage>
                                </a>
                            </template>
                        </el-table-column>
                        <el-table-column prop="name" :label="trans('bank.table.name')" sortable="custom">
                            <template slot-scope="scope">
                                <a @click.prevent="goToEdit(scope)" href="#">
                                    {{ scope.row.namaBank }}
                                </a>
    ...
    <div class="modal fade bank-form" tabindex="-1" role="dialog" aria-labelledby="myLargeModalLabel" aria-hidden="true">
        <div class="modal-dialog">
            <div class="modal-content">
                <el-form ref="form_bank" :model="form_bank" label-width="120px" label-position="top" v-loading="loadingForm" @keydown="form.errors.clear($event.target.name);">
                    ...
                    <div class="modal-body">
                        <div class="row">
                            <div class="col-md-12">
                                <center>
                                    <el-form-item :class="{'el-form-item is-error': form.errors.has('logo') }">
                                        <upload-avatar :onSuccess="handleAvatarSuccess" :beforeUpload="beforeAvatarUpload" :image="form_bank.logo_preview">
                                        </upload-avatar>
                                        <div class="el-form-item__error" v-if="form.errors.has('logo_preview')" v-text="form.errors.first('logo_preview')"></div>
                                    </el-form-item>
                                </center>
                            </div>
                            <div class="col-md-12">
                                <el-form-item :label="trans('bank.form.name')" :class="{'el-form-item is-error': form.errors.has('name') }" prop="name" :rules="rules.required">
                                    <el-input v-model="form_bank.name" size="small"></el-input>
                                    <div class="el-form-item__error" v-if="form.errors.has('name')" v-text="form.errors.first('name')"></div>
                                </el-form-item>
                            </div>
                        </div>
                    </div>
                    <div class="modal-footer">
                        <div class="pull-right">
                            <el-form-item>
                                <el-button type="primary" @click="onSubmit('form_bank')" :loading="loadingForm">
                                    {{ trans('core.save') }}
                                </el-button>
...
export default {
    ...
    methods: {
        queryServer(customProperties) {
            ...
            axios.get(route('admin.configuration.bank.list', _.merge(properties, customProperties)),properties)
                .then((response) => {
                ...
                if (valid) {
                    Swal.fire({
                        ...
                        if (result.value) {
                            ...
                            this.form.post(route('api.configuration.bank.submit'))
                                .then((response) => {
        ...
        goToEdit(scope) {
            this.loadingForm = true;
            this.goToCreate();
            axios.post(route('api.configuration.bank.find', {
