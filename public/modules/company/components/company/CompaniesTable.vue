    ...
    <div class="container">
        <div class="card card-outline-info">
            <div class="card-header text-white">
                ...
                <el-button plain size="small" icon="el-icon-plus" class="pull-right" @click.prevent="goToCreate()">{{trans('companies.button.create company')}}</el-button>
            </div>
            <div class="card-body">
                <div class="sc-table">
                    <div class="tool-bar row" style="padding-bottom: 20px;">
                        <div class="col-md-8">
                            <div class="row">
                                <div class="col-md-6">
                                    <el-select v-model="typeSelected" @change="fetchData" size="small" placeholder="Select">
                    ...
                    <el-table :data="data" stripe style="width: 100%" ref="pageTable" v-loading.body="tableIsLoading" @sort-change="handleSortChange">
                        ...
                        <el-table-column prop="rating" :label="trans('companies.table.rating.rating')" sortable="custom">
                            <template slot-scope="scope">
                                <el-rate :value="scope.row.rating / 10" disabled text-color="#ff9900" :max="1" show-score :score-template="scope.row.rating.toString()">
                                </el-rate>
                            </template>
                        </el-table-column>
                        ...
                        <el-table-column prop="actions" :label="trans('core.table.actions')">
                            <template slot-scope="scope">
                                <el-button-group>
                                    <el-button size="mini" @click.prevent="goToEdit(scope)" v-if="scope.row.urls.edit_url" icon="el-icon-edit">
                                    </el-button>
                                    <delete-button :scope="scope" :rows="data" v-on:onDelete="tableIsLoading = $event" v-on:onDeleteSuccess="tableIsLoading = !$event"></delete-button>
                                </el-button-group>
                            </template>
                        </el-table-column>
                    </el-table>
...
export default {
    ...
    methods: {
        queryServer(customProperties) {
            ...
            axios.get(route('api.company.company.pagination'), properties)
                .then((response) => {
