                    ...
                    <div class="card-body">
                        <div class="row">
                            <div class="col-md-3 pb-3">
                                <el-select class="" size="mini" v-model="filterKet" filterable @change="filterLog">
                                    <el-option v-for="item in filter" :value="item.value" :label="item.label" :key="item.value"></el-option>
                                </el-select>
                            </div>
                            <div class="col-md-4">
                                <el-select v-if="user_role == 'Super Admin'" v-model="companiesKet" filterable size="mini" @change="filterLog">
                            ...
                            <div class="col-md-5">
                                <div class="btn-group float-right" style="margin: 0 15px 15px 0;">
                                    <el-input prefix-icon="el-icon-search" size="small" @keyup.enter="search" v-model="searchQuery">
                        ...
                        <div class="row">
                            <div class="col-md-12">
                                <el-table :data="logTable" height="420" style="width: 100%" ref='logTable' v-loading.body="tableIsLoading">
                                    <!-- <el-table-column :label="trans('users.table.no')" width="50" fixed>
                                        <template slot-scope="scope">
                                            {{scope.$index+1}}
                                        </template>
                                    </el-table-column> -->
                                    <el-table-column fixed prop="created_at" :label="trans('users.table.tgl')" width="200">
                                        <template slot-scope="scope">
                                    ...
                                    <el-table-column :label="trans('users.table.ket')">
                                        <template slot-scope="scope">
                                            <div style="padding-top:5px;">
                                                ...
                                                <span class="search-regex" style="font-size:10px;display:block;">
                                                    <b>{{scope.row.nama_user}} &nbsp; </b>
                                                    <span v-if="user_role=='Super Admin'">
                                                        ({{scope.row.nama_perusahaan}})
                                                    </span>
                                                </span>
                                                <span style="font-size:10px;">{{scope.row.deskripsi}}</span>
                                            </div>
                                        </template>
                                    </el-table-column>
                                </el-table>
...
export default {
    ...
    methods: {
        queryServer(customProperties) {
            ...
            axios.get(route('api.user.log.index'), _.merge(properties, customProperties))
                .then((response) => {
        ...
        handleScroll() {
            this.$refs.logTable.bodyWrapper.addEventListener('scroll', (list) => {
                ...
                if (totalHeight >= this.heightScroll - selisih && lengthDataLog < this.meta.total) {
                    this.tableIsLoading = true;
